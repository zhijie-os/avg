...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_action_magnitude_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_action_magnitude_blocks_aba_joint.csv",
        )
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "adaptation_active,adaptation_remaining,"
                "predictor_lr,active_slow_ema,"
        ...
        with open(self.regime_log_path, "w") as f:
            f.write(
                "step,block_evidence,block_familiarity,"
        ...
        self.base_predictor_lr = cfg.predictor_lr
        self.slow_ema = cfg.slow_ema
        self.post_change_adaptation_steps = (
            cfg.post_change_adaptation_steps
        )
        self.adaptation_lr_multiplier = (
            cfg.adaptation_lr_multiplier
        )
        self.adaptation_slow_ema = cfg.adaptation_slow_ema
        self.adaptation_remaining = 0
        ...
        if self.post_change_adaptation_steps < 0:
            raise ValueError(
                "Require post_change_adaptation_steps >= 0"
            )

        if self.adaptation_lr_multiplier <= 0.0:
            raise ValueError(
                "Require adaptation_lr_multiplier > 0"
            )

        if not (0.0 <= self.adaptation_slow_ema < 1.0):
            raise ValueError(
                "Require 0 <= adaptation_slow_ema < 1"
            )

        if self.adaptation_slow_ema > self.slow_ema:
            raise ValueError(
                "adaptation_slow_ema should be <= slow_ema so "
                "the slow reference tracks fast more aggressively "
                "after a detected change"
            )
    ...
    def ema_update_slow_from_fast(self, ema):
    ...
    def set_fast_predictor_lr(self, lr):
        """Set the learning rate of every fast-predictor optimizer."""

        lr = float(lr)

        for optimizer in self.fast_pred_opts:
            for param_group in optimizer.param_groups:
                param_group["lr"] = lr

    def start_post_change_adaptation(self):
        """
        Enter a temporary greedy predictor-adaptation phase.

        The fast predictor learns with a larger optimizer learning rate,
        while the slow reference tracks fast with a smaller EMA coefficient.
        The regime detector remains disarmed throughout this phase.
        """

        self.adaptation_remaining = int(
            self.post_change_adaptation_steps
        )

        if self.adaptation_remaining <= 0:
            self.set_fast_predictor_lr(
                self.base_predictor_lr
            )
            return
        ...
        self.set_fast_predictor_lr(adaptation_lr)

        print(
            f"GREEDY ADAPTATION started at step={self.steps}: "
            f"steps={self.adaptation_remaining}, "
            f"fast_lr={adaptation_lr:.6g}, "
            f"slow_ema={self.adaptation_slow_ema:.6f}"
        )

    def finish_post_change_adaptation(self):
        """Return predictor timescales to their normal settings."""
        ...
        print(
            f"GREEDY ADAPTATION finished at step={self.steps}: "
            f"fast_lr={self.base_predictor_lr:.6g}, "
            f"slow_ema={self.slow_ema:.6f}"
        )
    ...
    def update(
        ...
        **kwargs,
            ...
            if self.block_count >= self.block_size:
                ...
                if block_valid:
                        ...
                        if (
                            ...
                            with open(
                                ...
                                f.write(
                                    ...
                                    f"{self.post_change_adaptation_steps}\n"
                                )

                            # A confirmed change starts a temporary
                            # greedy adaptation phase. We do NOT reset the
                            # predictors or familiarity counts: the fast
                            # predictor simply learns faster, while the slow
                            # EMA catches up more aggressively.
                            self.start_post_change_adaptation()
                    ...
                    # -------------------------------------
                        ...
                        # Never rearm while the predictors are deliberately
                        # adapting quickly to a change that was already
                        # confirmed. Otherwise a later wave of the SAME
                        # adaptation can be counted as a second change.
                        if self.adaptation_remaining > 0:
                            self.rearm_streak = 0
                ...
                with open(
                    ...
                    f.write(
                        ...
                        f"{int(self.adaptation_remaining > 0)},"
                        f"{self.adaptation_remaining}\n"
                    )
        ...
        #
        # IMPORTANT:
        # A confirmed detector alarm does not reset either ensemble.
        # Instead it temporarily accelerates fast learning and makes
        # the slow EMA track fast more aggressively.
        # =================================================
        ...
        if self.warmup_remaining > 0:
            ...
            if self.adaptation_remaining > 0:
                slow_ema_used = self.adaptation_slow_ema
            else:
                slow_ema_used = self.slow_ema

            self.ema_update_slow_from_fast(
                slow_ema_used
            )

        # One environment transition of greedy adaptation has now been
        # consumed. Restore the normal fast LR only after both fast and
        # slow have received the accelerated update on this transition.
        if self.adaptation_remaining > 0:
            self.adaptation_remaining -= 1

            if self.adaptation_remaining == 0:
                self.finish_post_change_adaptation()

        current_predictor_lr = float(
            self.fast_pred_opts[0]
            .param_groups[0]["lr"]
        )
        ...
        return {
            ...
            "adaptation_active": self.adaptation_remaining > 0,
            "adaptation_remaining": self.adaptation_remaining,
            "predictor_lr": current_predictor_lr,
            "active_slow_ema": slow_ema_used,
    ...
    def save(self, model_dir, unique_str):
        ...
        model = {
            ...
            "post_change_adaptation_steps": (
                self.post_change_adaptation_steps
            ),
            "adaptation_lr_multiplier": (
                self.adaptation_lr_multiplier
            ),
            "adaptation_slow_ema": self.adaptation_slow_ema,
            "adaptation_remaining": self.adaptation_remaining,
            "rate_warmup_remaining": self.rate_warmup_remaining,
            "detector_armed": self.detector_armed,
            "initial_arming_complete": self.initial_arming_complete,
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-fastslow-state-action-magnitude-greedy-adaptation"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Number of transitions after a confirmed regime change "
            "during which predictor adaptation is accelerated and the "
            "detector remains disarmed"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Multiplier applied to the fast-predictor learning rate "
            "during post-change greedy adaptation"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "EMA coefficient for the slow predictor during post-change "
            "adaptation; smaller than --slow_ema means faster catch-up"
        ),
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_fastslow_state_action_magnitude_evidence"
