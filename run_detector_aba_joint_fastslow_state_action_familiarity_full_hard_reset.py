...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_action_familiarity_full_hard_reset_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_action_familiarity_full_hard_reset_blocks_aba_joint.csv",
        )
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                "step,detector_epoch,fast_slow_gap,fast_wins,"
        ...
        with open(self.block_trace_path, "w") as f:
            f.write(
                "block_end_step,detector_epoch,block_valid,block_win_rate,"
                "block_familiarity,block_weight_sum,"
                "q_short,q_long,rate_gap,"
                "detector_armed,regime_change,"
                "positive_streak,initial_stabilization_streak,rearm_streak,"
        ...
        with open(self.regime_log_path, "w") as f:
            f.write(
                "step,detector_epoch,block_win_rate,block_familiarity,"
        ...
        # Detector epoch 0 preserves the original experiment behavior.
        # Each trusted alarm starts a brand-new detector epoch.
        self.detector_epoch = 0

        # Detector-local Poisson bootstrap RNG. Epoch 0 uses cfg.seed,
        # matching the original global np.random Poisson stream because
        # no other global NumPy RNG draws occur during learning.
        ...
        # for several COMPLETE blocks in a row.
        #
        # A declared alarm is TRUSTED. It terminates the current
        # detector epoch. All detector-specific learned/history state
        # is then hard-reset:
        #   - fast predictor weights
        #   - fast predictor Adam states
        #   - slow predictor weights
        #   - Poisson bootstrap RNG stream
        #   - state-action familiarity hash/count/normalization state
        #   - block accumulators, q_short/q_long, and all arm/streak state
        #
        # The actor, critic, environment normalization, and RL learning
    ...
    def hard_reset_all_detector_state(self):
        """
        Start a completely fresh detector epoch after a trusted alarm.

        EVERYTHING that belongs to the detector is rebuilt or reset.
        Nothing belonging to the control learner is changed.

        Fresh detector state:
          1) random fast-predictor weights,
          2) fresh Adam optimizer state for every fast predictor,
          3) fresh slow predictors copied exactly from the new fast ones,
          4) a fresh detector-local Poisson(1) bootstrap RNG,
          5) a brand-new state-action familiarity tracker, including
             new LSH projections/offsets, zero counts, and new
             normalization warmup,
          6) empty block accumulators,
          7) q_short/q_long cleared,
          8) arming/stabilization/persistence state cleared,
          9) predictor comparison warmup restarted.

        The next transition is the first sample of the new detector epoch.
        """

        self.detector_epoch += 1

        # ----------------------------------------------------
        # Fresh detector-local random streams.
        #
        # Use deterministic epoch-specific seeds for reproducibility.
        # torch.random.fork_rng preserves the global Torch RNG state,
        # so resetting the detector does NOT perturb policy sampling.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Completely fresh fast predictor ensemble.
        # ----------------------------------------------------
        ...
        # Fresh Adam states. No moments or step counters survive.
        self.fast_pred_opts = [
            torch.optim.Adam(
                predictor.parameters(),
        ...
        # ----------------------------------------------------
        # Fresh slow ensemble: identical to fast at epoch start.
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Completely fresh familiarity detector component.
        #
        # Reconstructing the object resets not only counts and
        # normalization, but also the random LSH projections,
        # offsets, and hash coefficients.
        # ----------------------------------------------------

        self.state_action_familiarity = (
            StateActionFamiliarityTracker(
                dim=(
                    self.cfg.obs_dim
                    + self.cfg.action_dim
                ),
                seed=familiarity_seed,
                norm_warmup_steps=(
                    self.cfg.familiarity_norm_warmup
                ),
                num_tables=(
                    self.cfg.familiarity_num_tables
                ),
                projections_per_table=(
                    self.cfg.familiarity_projections_per_table
                ),
                num_buckets=(
                    self.cfg.familiarity_num_buckets
                ),
                hash_width=(
                    self.cfg.familiarity_hash_width
                ),
                count_scale=(
                    self.cfg.familiarity_count_scale
                ),
                input_clip=(
                    self.cfg.familiarity_input_clip
                ),
            )
        )

        # ----------------------------------------------------
        # Completely fresh detector statistics / state machine.
        # ----------------------------------------------------

        self.warmup_remaining = (
            self.comparison_warmup
        )
        ...
        self.rate_warmup_remaining = (
            self.rate_warmup_blocks
        )
        ...
        print(
            f"DETECTOR FULL HARD RESET at step={self.steps}: "
            f"new_epoch={self.detector_epoch}, "
            f"predictor_init_seed={predictor_init_seed}, "
            f"poisson_seed={bootstrap_seed}, "
            f"familiarity_seed={familiarity_seed}, "
            f"comparison_warmup={self.comparison_warmup}, "
            f"familiarity_warmup="
            f"{self.cfg.familiarity_norm_warmup}"
        )
    ...
    def update(
        ...
        **kwargs,
                ...
                if block_valid:
                    ...
                    # -------------------------------------
                        ...
                        if (
                            ...
                            # Trust the alarm. Finish scoring/training/
                            # logging this transition with the old detector,
                            # then hard-reset EVERY detector component.
                    ...
                    # -------------------------------------
                    # Fallback post-detection rearming.
                    #
                    # Under the trusted-alarm design this branch is not
                    # normally used after a confirmed alarm because the
                    # alarm starts a completely fresh detector epoch and
                    # initial_arming_complete is reset to False.
                    # -------------------------------------
        ...
        #    streaming transition using detector-epoch-local
        #    online Poisson(1) bootstrap weights.
        # =================================================
        ...
        #
        # IMPORTANT:
        # A trusted alarm ends this entire detector epoch.
        # The current transition completes under the old detector;
        # after logging, every detector component is hard-reset.
        # =================================================
        ...
        # =================================================
        # 6. Trusted-alarm full detector restart
        # =================================================

        if reset_detector_after_step:
            self.hard_reset_all_detector_state()
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-fastslow-state-action-familiarity-full-hard-reset"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Steps at the beginning of EVERY detector epoch during "
            "which slow is hard-synced to fast and detection is disabled"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "State-action samples at the beginning of EVERY detector "
            "epoch used to estimate and freeze familiarity normalization"
        ),
    )
