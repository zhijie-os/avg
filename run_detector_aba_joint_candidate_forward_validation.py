...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_candidate_forward_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_candidate_forward_blocks_aba_joint.csv",
        )
        ...
        self.candidate_log_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_candidate_tests_aba_joint.csv",
        )
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "candidate_last_score,candidate_threshold,"
        ...
        with open(self.regime_log_path, "w") as f:
            f.write(
                "step,suspicion_step,validation_score,validation_threshold,"
                "validation_streak,validation_blocks\n"
            )

        with open(self.candidate_log_path, "w") as f:
            f.write(
                "step,event,suspicion_step,phase,validation_score,"
        ...
        # -------------------------------------------------
        # Held-forward candidate validation
        # -------------------------------------------------
        # The fast/slow magnitude detector is only a CHEAP SUSPICION
        # trigger. A suspected change is confirmed only if a temporary
        # candidate, trained on earlier post-suspicion transitions,
        # generalizes better than an identical frozen reference on LATER
        # transitions that the candidate has not trained on.
        ...
        self.candidate_lr = (
            cfg.predictor_lr
            * cfg.candidate_lr_multiplier
        )

        # Dedicated candidate RNG keeps temporary candidate tests from
        # perturbing the bootstrap randomness of the main fast ensemble.
        # False suspicions therefore do not change the learner trajectory.
        self.candidate_rng = np.random.RandomState(
            cfg.seed + 104729
        )

        # Online null calibration. Only validation scores from REJECTED
        # candidate tests are added, so confirmed regime changes do not
        # contaminate the stationary baseline.
        self.forward_null_stats = RunningStats()

        # At most one candidate test exists at a time: fixed memory O(1)
        # with respect to stream length and number of regimes.
        self.candidate_active = False
        self.candidate_reference = None
        self.candidate_predictors = None
        self.candidate_opts = None
        ...
        if self.candidate_lr <= 0.0:
            raise ValueError("Require candidate learning rate > 0")
    ...
    # =====================================================
    # Held-forward candidate validation helpers
    # =====================================================
    ...
    def start_candidate_test(self):
        """
        Start a fixed-memory held-forward test from the CURRENT fast model.

        Reference and candidate begin IDENTICAL. The reference is frozen.
        The candidate trains on future adaptation segments only. Validation
        segments are scored BEFORE any candidate update and are never used
        to train the candidate, which prevents same-sample adaptation from
        ...
        self.candidate_reference = copy.deepcopy(
            self.fast_predictors
        )
        self._freeze_module_list(
            self.candidate_reference
        )

        self.candidate_predictors = copy.deepcopy(
            self.fast_predictors
        )
        ...
        print(
            f"CANDIDATE TEST started at step={self.steps}: "
    ...
    def _train_candidate(
        ...
        """One strict-streaming candidate update on the current sample."""

        for predictor, optimizer in zip(
            self.candidate_predictors,
            self.candidate_opts,
        ):
            k_n_t = self.candidate_rng.poisson(1.0)
    ...
    def reject_candidate_test(self):
        ...
        print(
            f"CANDIDATE TEST rejected at step={self.steps}: "
    ...
    def confirm_candidate_test(self):
        ...
        # Promote the candidate to the main fast ensemble. Fresh Adam state
        # avoids carrying optimizer moments from the old comparison epoch.
        for fast_predictor, candidate_predictor in zip(
            self.fast_predictors,
            self.candidate_predictors,
        ):
            fast_predictor.load_state_dict(
                candidate_predictor.state_dict()
            )
    ...
    def update_candidate_test(
        ...
        # HELD-FORWARD scoring: candidate and reference are evaluated on the
        # transition BEFORE any candidate update, and validation transitions
        # are never used to train the candidate.
        log_p_candidate = self._ensemble_log_prob(
            self.candidate_predictors,
            raw_obs,
            action,
            target,
        )
        log_p_reference = self._ensemble_log_prob(
            self.candidate_reference,
            raw_obs,
            action,
            target,
        ...
        forward_gap = float(
            np.clip(
                log_p_candidate - log_p_reference,
        ...
        print(
            f"CANDIDATE validation at step={self.steps}: "
        ...
        # still-later samples that have not been used for candidate updates.
        self.candidate_phase = "adapt"
        self.candidate_phase_step = 0
        self._reset_candidate_validation_accumulator()
    ...
    def save(self, model_dir, unique_str):
        ...
        model = {
            ...
            "candidate_reference": (
                [
                    predictor.state_dict()
                    for predictor in self.candidate_reference
                ]
            ...
            "candidate_predictors": (
                ...
                if self.candidate_predictors is not None
                else None
            ),
            "rate_warmup_remaining": self.rate_warmup_remaining,
            "detector_armed": self.detector_armed,
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-candidate-forward-validation"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    # =====================================================
    # Held-forward candidate confirmation
    # =====================================================
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Transitions used to train the temporary candidate before "
            "each held-forward validation segment"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Future transitions scored by candidate and frozen reference; "
            "the candidate is NOT trained on these samples"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Minimum familiarity-weighted mean candidate-minus-reference "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "candidate-minus-reference log-likelihood gaps"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Temporary candidate learning rate as a multiplier of "
            "--predictor_lr"
        ),
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_candidate_forward_validation"
