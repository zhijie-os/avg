    ...
    def __init__(self, cfg):
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "state_residual_mean_sq,reward_residual_sq,residual_vector,"
    ...
    def update(
        ...
        **kwargs,
        ...
        if self.warmup_remaining > 0:
            ...
            if self.change_score > self.detector_h:
                ...
                if (
                    ...
                    alarm_residual_vector_str = "[" + " ".join(
                        f"{v:.8f}" for v in current_residual
                    ) + "]"
                else:
                    alarm_residual_vector_str = "[]"
                ...
                with open(
                    ...
                    f.write(
                        ...
                        f"residual_vector={alarm_residual_vector_str}, "
        ...
        if self.steps % self.detector_log_interval == 0:
            ...
            # Log the full normalized residual vector only for suspicious
            # trusted samples. This keeps the CSV compact while preserving
            # the residual direction needed to diagnose positive evidence.
            if (
                current_residual is not None
                and np.all(np.isfinite(current_residual))
                and L_t > 0.0
                ...
                residual_vector_str = (
                    '"'
                    + " ".join(f"{v:.8f}" for v in current_residual)
                    + '"'
                )
            else:
                residual_vector_str = '""'
