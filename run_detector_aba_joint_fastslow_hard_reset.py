...
class StateActionFamiliarityTracker:
    ...
    def reset_epoch(self):
        """
        Reset all history-dependent familiarity state for a new
        detector epoch while keeping the fixed random hash functions.

        This gives the new accepted regime a clean state-action support
        estimate without allocating memory that grows with the number
        of regimes.
        """
...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_hard_reset_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_hard_reset_blocks_aba_joint.csv",
        )
        ...
        # Number of accepted detector epochs. Epoch 0 is the initial
        # environment segment. A confirmed alarm starts a fresh epoch.
    ...
    def hard_reset_detector_epoch(self):
        """
        Start a completely fresh detector epoch after a trusted alarm.

        Only auxiliary change-detection state is reset. Actor, critic,
        TD-error scaler, and all control-learning state are untouched.

        Reset:
          - fast probabilistic predictor ensemble: random reinitialization
          - fast predictor Adam optimizers: fresh state
          - slow predictor ensemble: exact copy of the new fast ensemble
          - state-action familiarity counts and normalization statistics
          - block accumulators, short/long evidence estimates, and streaks
          - predictor/familiarity calibration warmup

        Fixed random LSH projections are retained because they contain no
        learned regime information.
        """
        ...
        # -------------------------------------------------
        # Slow starts IDENTICAL to the newly initialized fast
        # ensemble and is frozen as an EMA reference.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Fresh regime-local familiarity state.
        # -------------------------------------------------

        self.state_action_familiarity.reset_epoch()

        # -------------------------------------------------
        # Fresh comparison / detector state.
        # -------------------------------------------------
        ...
        print(
            f"DETECTOR HARD RESET at step={self.steps}: "
            f"starting epoch={self.detector_epoch}, "
            f"predictor_warmup={self.comparison_warmup}, "
            f"familiarity_warmup="
            f"{self.state_action_familiarity.norm_warmup_steps}"
        )
    ...
    def update(
        ...
        **kwargs,
        ...
        regime_change = False
        reset_detector_after_step = False
                ...
                if block_valid:
                    ...
                    # -------------------------------------
                        ...
                        if (
                            ...
                            # Trust the confirmed alarm. The current
                            # transition is allowed to finish the old
                            # detector update/logging, then ALL auxiliary
                            # detector state is restarted from scratch.
                            #
                            # The policy/critic are NOT reset.
                            reset_detector_after_step = True
                    ...
                    # -------------------------------------
                    # Fallback rearming path.
                    #
                    # With hard-reset-on-alarm this branch is not used
                    # during the normal path: after a trusted alarm the
                    # next transition belongs to a fresh detector epoch
                    # with initial_arming_complete=False.
                    # -------------------------------------
        ...
        #
        # IMPORTANT:
        # A confirmed detector alarm ends the current detector epoch.
        # The old ensemble completes this transition, after which the
        # auxiliary predictors and familiarity state are reset.
        # =================================================
        ...
        # =================================================
        # 6. Trusted-alarm detector restart
        #
        # Do this only AFTER the alarm transition has been scored,
        # learned from, and logged by the old detector epoch.
        # The next transition therefore starts from completely fresh
        # auxiliary predictors and familiarity statistics.
        # =================================================

        if reset_detector_after_step:
            self.hard_reset_detector_epoch()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Steps at the start of every detector epoch during which "
            "slow is hard-synced to fast and detection is disabled"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "State-action samples at the start of every detector epoch "
            "used to estimate and then freeze familiarity normalization"
        ),
    )
