...
class LocalFrozenPredictorDetector:
    ...
    State-action space is partitioned by Euclidean random-projection LSH (E2LSH-style)
    in frozen-normalized state-action space. Every bucket owns its own
    affine RLS model, so an update in one bucket cannot interfere with
    any other bucket.
    ...
    def __init__(
        ...
        # Maximum number of E2LSH cells that may allocate local models.
        # This preserves a fixed memory bound without forcing unrelated
        # E2LSH signatures to collide through modulo hashing.
        self.num_buckets = int(num_buckets)
        self.lsh_width = float(lsh_width)
        self.lsh_projections = int(lsh_projections)
        ...
        if self.lsh_width <= 0:
            raise ValueError(
                "detector_lsh_width must be > 0"
            )

        if self.lsh_projections < 1:
            raise ValueError(
                "detector_lsh_projections must be >= 1"
            )
        ...
        # -------------------------------------------------
        # Euclidean random-projection LSH (E2LSH-style).
        #
        # Each row is a random unit direction. With x already
        # normalized, lsh_width is therefore directly interpretable
        # in normalized-coordinate units along that direction.
        #
        #     h_j(x) = floor((a_j^T x + b_j) / w)
        #
        # Nearby Euclidean points are more likely to share the full
        # integer signature. Unlike SimHash, this does not use angle
        # alone as the locality notion.
        # -------------------------------------------------

        self.lsh_projection = rng.standard_normal(
            (
        ...
        self.lsh_projection /= np.maximum(
            projection_norms,
            1e-12,
        )

        self.lsh_offset = rng.uniform(
            0.0,
            self.lsh_width,
            size=self.lsh_projections,
        )

        # E2LSH signatures are mapped to sequential integer IDs only
        # for logging/storage. No modulo hashing is used, so different
        # signatures never alias while capacity remains available.
        self.signature_to_bucket_id = {}
        ...
        # At most num_buckets local E2LSH cells can allocate models.
        self.buckets = {}
    ...
    def _bucket_id(self, x_norm):
        # Euclidean random-projection LSH signature.
        projected = (
            self.lsh_projection
            @ x_norm
            + self.lsh_offset
        ...
        bucket_id = self.signature_to_bucket_id.get(
            signature
        )
        ...
        # Fixed-memory behavior: do not alias a brand-new E2LSH
        # signature onto an unrelated existing local model. Once the
        # configured capacity is full, simply leave unseen cells
        # unmodelled. Existing cells continue to work normally.
        if len(self.signature_to_bucket_id) >= self.num_buckets:
            if not self._capacity_warned:
                print(
                    "Detector E2LSH bucket capacity exhausted; "
                    "new unseen cells will be ignored."
                )
    ...
    def process(
        ...
        # Capacity overflow is intentionally not forced into an
        # unrelated existing cell. The detector simply abstains on
        # this previously unseen E2LSH region.
...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector = LocalFrozenPredictorDetector(
            ...
            lsh_width=cfg.detector_lsh_width,
            lsh_projections=cfg.detector_lsh_projections,
    ...
    def save(self, model_dir, unique_str):
        ...
        model = {
            ...
            "detector": {
                ...
                "lsh_projection": self.detector.lsh_projection,
                "lsh_offset": self.detector.lsh_offset,
                "lsh_width": self.detector.lsh_width,
                "lsh_projections": self.detector.lsh_projections,
                "signature_to_bucket_id": self.detector.signature_to_bucket_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Maximum number of Euclidean-LSH local predictor cells "
            "that may allocate models"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "E2LSH bin width in frozen-normalized state-action space"
        ),
    )

    parser.add_argument(
        "--detector_lsh_projections",
        default=3,
        type=int,
        help=(
            "Number of unit random projections concatenated into each "
            "E2LSH signature"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=12345,
        type=int,
        help="Fixed seed for the detector's E2LSH projections/offsets",
    )
