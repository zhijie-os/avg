"""Oracle mixture: HalfCheetah gravity 1.0 -> 0.7 -> 1.0.

The upper-reference oracle.  One independent AVG learner per regime; only the
active learner acts and updates.  When regime A returns, its learner is
restored exactly where it was before regime B (weights, optimizers, and all
normalization/TD statistics).  This intentionally uses O(#regimes) memory and
is NOT the proposed final method.

Usage:
    python3 run_oracle_mixture.py
"""
