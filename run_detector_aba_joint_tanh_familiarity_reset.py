...
class StateActionFamiliarityTracker:
    ...
    The LSH mapping is fixed after the initial state-action normalization
    warmup.  Within each detected regime, we keep pseudo-counts and a
    running mean/variance of the *log pseudo-count*:

        u_t = log(1 + c_t)

    where c_t is the median pseudo-count across independent LSH tables.

    Relative familiarity is scored BEFORE the current transition updates
    either the count table or the running familiarity statistics:

        z^F_t = (u_t - mu_F) / sqrt(var_F + 1)

    The +1 regularizes the scale when a regime has only just started and
    its count variance is still near zero.  z^F_t is not capped at 1.

    The detector maps normalized familiarity through a centered tanh:

        w_t = 1 + tanh(z^F_t)

    so w_t is in (0, 2).  At the regime-local mean, w_t = 1.
    Below-mean familiarity is smoothly and nonlinearly suppressed toward 0;
    above-mean familiarity is smoothly amplified toward 2.  This avoids both
    the old hard cutoff max(0, z) and unbounded familiarity weights.
    """
    ...
    def __init__(
        ...
        # Frozen normalization used only for the LSH input coordinates.
        self.norm_count = 0
        ...
        # Regime-local statistics of log(1 + pseudo_count).
        self.familiarity_score_count = 0
        self.familiarity_score_mean = 0.0
        self.familiarity_score_M2 = 0.0
    ...
    def _familiarity_variance(self):
        if self.familiarity_score_count < 2:
            return 0.0

        return max(
            self.familiarity_score_M2
            / float(self.familiarity_score_count - 1),
            0.0,
        )

    def _update_familiarity_stats(self, value):
        value = float(value)
        self.familiarity_score_count += 1

        delta = value - self.familiarity_score_mean
        self.familiarity_score_mean += (
            delta / self.familiarity_score_count
        )
        delta2 = value - self.familiarity_score_mean
        self.familiarity_score_M2 += delta * delta2
    ...
    def observe(self, x):
        ...
        Return:
          pseudo_count,
          log_count,
          familiarity_mean_before_update,
          familiarity_std_before_update,
          familiarity_z,
          ready

        Query/scoring happens before current-transition count/stat updates.
        """
        ...
        if not self.ready:
            ...
            return 0, 0.0, 0.0, 1.0, 0.0, self.ready
        ...
        log_count = float(np.log1p(float(pseudo_count)))

        familiarity_mean = float(self.familiarity_score_mean)
        familiarity_variance = self._familiarity_variance()

        # Unit variance regularizer prevents pathological z-scores when
        # a newly reset regime still has nearly zero empirical variance.
        familiarity_std = float(
            np.sqrt(familiarity_variance + 1.0)
        )

        familiarity_z = float(
            (log_count - familiarity_mean)
            / familiarity_std
        )

        # Score before updating the regime-local familiarity statistics.
        self._update_familiarity_stats(log_count)
    ...
    def reset_counts(self):
        """
        Start a new regime-local familiarity reference while preserving
        the fixed LSH projections and frozen input normalization.
        """
        self.counts.fill(0)
        self.familiarity_score_count = 0
        self.familiarity_score_mean = 0.0
    ...
    def state_dict(self):
        return {
            ...
            "counts": self.counts,
            "familiarity_score_count": self.familiarity_score_count,
            "familiarity_score_mean": self.familiarity_score_mean,
            "familiarity_score_M2": self.familiarity_score_M2,
        }
...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "state_action_pseudo_count,familiarity_log_count,"
                "familiarity_mean,familiarity_std,familiarity_z,"
                "familiarity_weight,e_t,"
        ...
        with open(self.detector_block_path, "w") as f:
            f.write(
                ...
                "mean_familiarity_z,mean_familiarity_weight,"
                "mean_surprise,"
        ...
        #   z^F_t = standardized regime-local log pseudo-count
        #   w_t = 1 + tanh(z^F_t)
        #   e_t = clip(s_t, 0, s_max) * w_t
        #
        # Block evidence:
        #   E_k = (1/B) sum_{t in block k} e_t
        #
        # Familiarity-gated surprise residual CUSUM:
        #   G_k = (1/B) sum_t w_t * (s_t - mu_s - delta)
    ...
    def update(
        ...
        **kwargs,
        ...
        # Nonlinear relative-familiarity gate.
        #
        # First normalize regime-local log pseudo-count:
        #
        #   z^F_t = (log(1 + c_t) - mu_F) / sigma_F
        #
        # Then map it smoothly into a bounded multiplicative weight:
        #
        #   w_t = 1 + tanh(z^F_t)   in (0, 2)
        #
        # Interpretation:
        #   z^F_t < 0  -> below-normal familiarity -> suppress surprise
        #   z^F_t = 0  -> normal familiarity       -> weight 1
        #   z^F_t > 0  -> above-normal familiarity -> amplify surprise
        #
        # Unlike max(0, z), this does not completely discard slightly
        # below-mean state-action regions.  Unlike an unbounded mapping,
        # very large pseudo-counts cannot dominate the CUSUM.
        if familiarity_ready:
            familiarity_weight = float(
                1.0 + np.tanh(familiarity_z)
            )
        ...
        if self.warmup_remaining > 0:
            ...
            self.block_familiarity_sum += familiarity_z
            ...
            if self.block_count >= self.block_size:
                ...
                with open(
                    ...
                    f.write(
                        ...
                        f"{mean_block_familiarity_z:.8f},"
                        f"{mean_block_weight:.8f},"
                ...
                if regime_change:
                    with open(
                        ...
                        f.write(
                            ...
                            f"mean_familiarity_z="
                            f"{mean_block_familiarity_z:.8f}, "
                            f"mean_familiarity_weight="
                            f"{mean_block_weight:.8f}, "
                            f"mean_surprise="
                            f"{mean_block_surprise:.8f}, "
        ...
        if self.steps % self.detector_log_interval == 0:
            ...
            with open(
                ...
                f.write(
                    ...
                    f"{familiarity_log_count:.8f},"
                    f"{familiarity_mean:.8f},"
                    f"{familiarity_std:.8f},"
                    f"{familiarity_z:.8f},"
        ...
        return {
            ...
            "familiarity_log_count": familiarity_log_count,
            "familiarity_mean": familiarity_mean,
            "familiarity_std": familiarity_std,
            "familiarity_z": familiarity_z,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Minimum mean positive relative-familiarity weight required "
            "to initialize/update the surprise baseline"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Deprecated compatibility argument; relative-familiarity "
            "the tanh-normalized familiarity gate no longer uses F^gamma."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Deprecated compatibility argument; relative-familiarity "
            "z-scoring no longer uses count saturation."
        ),
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_relative_familiarity_gradient_diagnostics"
