...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_adaptive_null_trace_aba_joint.csv",
        )
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "p0_hat,p1_hat,bernoulli_llr,cusum_W,"
                "detector_active,regime_change,p0_update_enabled,"
                "initial_warmup_remaining,calibration_remaining,"
        ...
        # -------------------------------------------------
        # Adaptive-null Bernoulli likelihood-ratio CUSUM
        #
        # I_t = 1 if the fast predictor explains the current
        # transition better than the slow reference.
        #
        # Instead of assuming a fixed null probability p0=0.5,
        # estimate the ordinary fast-win rate online:
        #
        #   H0: I_t ~ Bernoulli(p0_hat)
        #   H1: I_t ~ Bernoulli(p1_hat)
        #
        # where
        #
        #   p1_hat = min(p0_hat + p1_delta, p1_max).
        #
        # The null is calibrated after startup warmup and then
        # slowly adapted only while CUSUM suspicion is low.
        # -------------------------------------------------

        self.detector_h = cfg.detector_h

        self.p0_init = cfg.p0_init
        self.p0_ema = cfg.p0_ema
        self.p0_min = cfg.p0_min
        self.p0_max = cfg.p0_max

        self.p1_delta = cfg.p1_delta
        self.p1_max = cfg.p1_max

        self.null_calibration_steps = cfg.null_calibration_steps
        self.post_change_calibration_steps = (
            cfg.post_change_calibration_steps
        )
        self.null_prior_strength = cfg.null_prior_strength
        self.p0_freeze_fraction = cfg.p0_freeze_fraction

        if not (0.0 < self.p0_init < 1.0):
            raise ValueError(
                "Require 0 < p0_init < 1"
            )

        if not (0.0 <= self.p0_ema < 1.0):
            raise ValueError(
                "Require 0 <= p0_ema < 1"
            )
        ...
        if not (
            ...
            raise ValueError(
                "Require 0 < p0_min < p0_max < 1"
            )

        if self.p1_delta <= 0.0:
            raise ValueError(
                "Require p1_delta > 0"
            )

        if not (0.0 < self.p1_max < 1.0):
            raise ValueError(
                "Require 0 < p1_max < 1"
            )

        if self.p1_max <= self.p0_max:
            raise ValueError(
                "Require p1_max > p0_max"
            )

        if self.null_calibration_steps <= 0:
            raise ValueError(
                "Require null_calibration_steps > 0"
            )

        if self.post_change_calibration_steps <= 0:
            raise ValueError(
                "Require post_change_calibration_steps > 0"
            )

        if self.null_prior_strength < 0.0:
            raise ValueError(
                "Require null_prior_strength >= 0"
            )

        if not (0.0 <= self.p0_freeze_fraction < 1.0):
            raise ValueError(
                "Require 0 <= p0_freeze_fraction < 1"
            )

        self.cusum_W = 0.0

        # Initial startup warmup: slow is hard-synchronized to fast.
        self.warmup_remaining = self.comparison_warmup

        # After startup warmup, detector stays disabled while the
        # fast/slow pair establishes its ordinary stationary
        # fast-win rate. The same recalibration is used after a
        # declared change.
        self.calibration_remaining = self.null_calibration_steps
        self.calibration_wins = 0.0
    ...
    def update(
        ...
        **kwargs,
        ...
        # =================================================
        # 2. Adaptive-null Bernoulli CUSUM
        #
        # IMPORTANT:
        # - Runs on EVERY transition.
        # - During startup warmup or null calibration, detector
        #   is disabled and W_t is held at zero.
        # - The current transition is scored using p0_hat from
        #   PREVIOUS transitions.
        # - p0_hat is updated only while CUSUM suspicion is low,
        #   so a real change is not immediately absorbed into H0.
        # =================================================

        detector_active = (
            self.warmup_remaining == 0
            and self.calibration_remaining == 0
        )

        regime_change = False
        bernoulli_llr = 0.0
        p0_update_enabled = False
        ...
        p1_for_score = float(
            min(
                self.p1_max,
                p0_for_score + self.p1_delta,
            )
        )

        # Numerical guard: H1 must remain strictly above H0.
        p1_for_score = max(
            p1_for_score,
            min(
                self.p1_max,
        ...
        if detector_active:
            ...
            if self.cusum_W >= self.detector_h:
                ...
                print(
                    ...
                    f"p0={p0_for_score:.3f}, "
                    f"p1={p1_for_score:.3f}, "
                    f"LLR={bernoulli_llr:.3f}"
                )
                ...
                # Slowly track the normal fast-win rate only while
                # the detector is not already suspicious.
                freeze_level = (
                    self.p0_freeze_fraction
                    * self.detector_h
                )

                if self.cusum_W <= freeze_level:
                    p0_update_enabled = True
            ...
            # During explicit null calibration, estimate the
            # ordinary fast-win rate directly. Startup warmup is
            # excluded because slow and fast are hard-synchronized.
            if (
                self.warmup_remaining == 0
                and self.calibration_remaining > 0
            ):
                self.calibration_wins += float(
                    fast_wins
                )
                self.calibration_count += 1

                prior_mass = self.null_prior_strength
                posterior_mass = (
                    prior_mass
                    + float(self.calibration_count)
                )
        ...
        #     slow <- fast exactly
        #
        # Null calibration and normal operation:
        #     slow <- beta * slow + (1-beta) * fast
        #
        # On a detected change:
        #     hard-sync ONCE, then disable detection while a
        #     fresh null fast-win rate is calibrated in the new
        #     regime. We do NOT hard-sync on every calibration
        #     step, which avoids repeatedly creating the same
        ...
        if self.steps % self.detector_log_interval == 0:
            ...
            with open(
                ...
                f.write(
                    ...
                    f"{int(regime_change)},"
                    f"{int(p0_update_enabled)},"
                    f"{self.warmup_remaining},"
                    f"{self.calibration_remaining},"
        ...
        return {
            ...
            "p0_update_enabled": p0_update_enabled,
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-fastslow-adaptive-null"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--null_calibration_steps",
        default=10_000,
        type=int,
        help=(
            "Steps after startup warmup used to estimate the normal "
            "fast-win probability before arming CUSUM"
        ),
    )

    parser.add_argument(
        "--post_change_calibration_steps",
        default=5_000,
        type=int,
        help=(
            "Detector-disabled steps after a declared change used to "
            "re-estimate the null fast-win probability"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="Prior mean for the adaptive null fast-win probability",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "EMA coefficient for p0_hat during low-suspicion "
            "normal operation"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.05,
        type=float,
        help="Lower numerical bound for adaptive p0_hat",
    )
    ...
    parser.add_argument(
        ...
        default=0.95,
        type=float,
        help="Upper numerical bound for adaptive p0_hat",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Alternative hypothesis is p1_hat = "
            "min(p0_hat + p1_delta, p1_max)"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0.995,
        type=float,
        help="Upper numerical bound for adaptive p1_hat",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Pseudo-count strength of p0_init during explicit "
            "null calibration"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Stop adapting p0_hat once CUSUM exceeds this fraction "
    ...
    parser.add_argument(
        ...
        default=100.0,
        type=float,
        help="Adaptive-null Bernoulli CUSUM threshold",
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_fastslow_adaptive_null"
