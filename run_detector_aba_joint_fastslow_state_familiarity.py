...
# =========================================================
# Fixed-memory state familiarity tracker
# =========================================================

class StateFamiliarityTracker:
    """
    Approximate visit counts for a continuous state space using a
    fixed-memory Euclidean locality-sensitive hash (E2LSH-style).

    The tracker deliberately uses ONLY S_t, not S_{t+1} or R_{t+1},
    so familiarity is determined before observing the transition
    outcome whose regime evidence we want to evaluate.

    During an initial warmup, raw-state mean/std are estimated with
    Welford updates. These statistics are then frozen so that bucket
    identities do not move over the lifetime of the run.

    For each frozen-normalized state, several independent random-grid
    hash tables are queried. The conservative pseudo-count is the
    minimum bucket count across tables. The current state is counted
    only AFTER its familiarity has been queried, preventing a sample
    from making itself look familiar.

    Memory is O(num_tables * num_buckets), independent of stream
    length and number of regimes.
    """
    ...
    def __init__(
        ...
        if self.state_clip <= 0.0:
            raise ValueError("Require familiarity_state_clip > 0")
        ...
        # Welford state for a frozen raw-state normalization.
        self.norm_count = 0
    ...
    def observe(self, raw_state):
        ...
        Familiarity is measured BEFORE the current state increments its
        hash buckets. The monotone weight is
        ...
        if x.shape[0] != self.dim:
            raise ValueError(
                f"Expected state dimension {self.dim}, got {x.shape[0]}"
            )
...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        # -------------------------------------------------
        # Fast-vs-slow state-familiarity detector logging
        # -------------------------------------------------
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_familiarity_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_fastslow_state_familiarity_blocks_aba_joint.csv",
        )
        ...
        with open(self.detector_trace_path, "w") as f:
            f.write(
                ...
                "state_pseudo_count,state_familiarity_weight,"
        ...
        # -------------------------------------------------
        # State-familiarity-weighted short-vs-long fast-win detector
        #
        # Per transition:
        #   I_t = 1[ log p_fast > log p_slow ]
        #
        # Aggregate correlated transitions into blocks, weighting each
        # sample by explicit historical state familiarity w_t:
        ...
        # Explicit count-based state familiarity. This replaces the
        # previous use of ensemble disagreement as a familiarity proxy.
        self.state_familiarity = StateFamiliarityTracker(
            dim=cfg.obs_dim,
            seed=cfg.seed + 7919,
            ...
            state_clip=cfg.familiarity_state_clip,
        )
    ...
    def update(
        ...
        **kwargs,
        ...
        # Keep an immutable NumPy copy for the count-based state
        # familiarity tracker BEFORE converting raw_obs to a tensor.
        ...
        # =================================================
        # 2. Explicit state familiarity
        #
        # IMPORTANT:
        # - Uses only raw S_t, never S_{t+1} or R_{t+1}.
        # - Query happens before the current state increments its count.
        # - The normalization is frozen after an initial warmup so hash
        #   bucket identities do not drift as the policy changes.
        # =================================================

        (
            state_pseudo_count,
            state_familiarity_weight,
            familiarity_ready,
        ) = self.state_familiarity.observe(
            raw_obs_for_familiarity
        )

        familiarity_warmup_remaining = (
            self.state_familiarity.warmup_remaining
        )

        # =================================================
        # 3. Familiarity-weighted block-rate detector
        #
        # Per transition, state familiarity supplies w_t in [0, 1].
        # Rare/novel states therefore contribute little to the block
        # evidence, while frequently revisited states contribute almost
        # fully. The weighted block fast-win rate is
        #
        #   b_k = sum_t w_t I_t / sum_t w_t.
        #
        ...
        # slow model on STATES WITH HISTORICAL SUPPORT?
        # =================================================
        ...
        if (
            ...
            self.block_weighted_wins += (
                float(state_familiarity_weight)
                * float(fast_wins)
            )
            ...
            if self.block_count >= self.block_size:
                ...
                if block_valid:
                    ...
                    # -------------------------------------
                    # Initial rate-estimation warmup. Only
                    # valid/familiar blocks count toward it.
                    # -------------------------------------
                    ...
                    # -------------------------------------
                        ...
                        if (
                            ...
                            # familiarity counts. Historical support
                            # remains available for recurring regimes.
                            self.detector_armed = False
                            self.positive_streak = 0
                            self.rearm_streak = 0

                    # -------------------------------------
                    # Disarmed: stricter stabilization test.
                    # Require both the two EMAs AND the current
                    # block itself to be near the long baseline.
                    # -------------------------------------
                ...
                # alarms coincide with low state familiarity.
                with open(
        ...
        if (
            ...
            with open(
                ...
                f.write(
                    ...
                    f"{fast_slow_gap:.8f},"
                    f"{fast_wins},"
                    f"{state_pseudo_count},"
                    f"{state_familiarity_weight:.8f},"
        ...
        return {
            ...
            "fast_wins": fast_wins,
            "state_pseudo_count": state_pseudo_count,
            "state_familiarity_weight": state_familiarity_weight,
    ...
    def save(self, model_dir, unique_str):
        ...
        model = {
            ...
            "state_familiarity": self.state_familiarity.state_dict(),
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-fastslow-state-familiarity"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    # =====================================================
    # Explicit count-based state familiarity
    # =====================================================
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Raw-state samples used to estimate and then freeze the "
            "normalization used by the familiarity hash"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Random-grid hash width in frozen standardized state space"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Clip frozen-standardized raw states before hashing"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Minimum average state-familiarity weight required for a "
            "block to update the regime detector"
        ),
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_fastslow_state_familiarity"
