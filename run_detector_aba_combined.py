...
# define the predictor network F_psi(S, A) -> S_hat, R_hat
class Predictor(nn.Module):
    def __init__(self, obs_dim, action_dim, device, n_hid=128):
        super().__init__()
        self.device = device

        # Shared representation of (s_t, a_t)
...
class AVG:
    def __init__(self, cfg):
        ...
        # initialize the predictor network F_psi
        self.predictor = Predictor(obs_dim=cfg.obs_dim, action_dim=cfg.action_dim, device=cfg.device, n_hid=cfg.nhid_predictor)
        ...
        # optimize the predictor with Adam
        self.pred_opt = torch.optim.Adam(self.predictor.parameters(),lr=cfg.predictor_lr)
    ...
    def update(self, obs, action, next_obs, reward, done, **kwargs):
        ...
        # predict S_hat, R_hat
        pred_next_obs, pred_reward = self.predictor(obs, action.detach())
        # e_t^P
        e_P = torch.sqrt(
            torch.mean(
        ...
        self.change_score = (
            ...
            * max(0.0, D_t - self.detector_tau)
        )

        regime_change = self.change_score > self.detector_h

        if regime_change:
            print(f"Regime change detected at step {self.steps}")
        ...
        state_pred_loss = torch.mean((next_obs - pred_next_obs) ** 2)
        ...
        reward_pred_loss = torch.mean(
            ((reward_tensor - pred_reward) / (self.reward_std + 1e-8)) ** 2
        )

        pred_loss = state_pred_loss + reward_pred_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nhid_predictor', default=128, type=int)
    parser.add_argument('--predictor_lr', default=1e-4, type=float)
    ...
    pkl_fpath = os.path.join(args.results_dir, "./{}_run_detector_aba_combined_seed-{}.pkl".format(args.env, args.seed))
