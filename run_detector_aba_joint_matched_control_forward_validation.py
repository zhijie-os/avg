...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_matched_control_forward_trace_aba_joint.csv",
        )

        self.block_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_matched_control_forward_blocks_aba_joint.csv",
        )
        ...
        self.candidate_log_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_matched_control_tests_aba_joint.csv",
        )

        with open(self.detector_trace_path, "w") as f:
            f.write(
                "step,fast_slow_gap,fast_wins,gap_evidence,"
                "state_action_pseudo_count,state_action_familiarity_weight,"
                "familiarity_ready,familiarity_warmup_remaining,"
                "block_completed,block_valid,block_evidence,"
                "block_familiarity,block_weight_sum,"
                "q_short,q_long,rate_gap,"
                "detector_armed,suspicion,regime_change,"
                "positive_streak,initial_stabilization_streak,rearm_streak,"
                "initial_arming_complete,rate_warmup_remaining,"
                "candidate_active,candidate_phase,candidate_phase_step,"
                "candidate_validation_blocks,candidate_positive_streak,"
                "rapid_minus_control_score,candidate_threshold,"
                "forward_null_n,forward_null_mean,forward_null_std,"
        ...
        with open(self.block_trace_path, "w") as f:
            f.write(
                "block_end_step,block_valid,block_evidence,"
                "block_familiarity,block_weight_sum,"
                "q_short,q_long,rate_gap,"
                "detector_armed,suspicion,regime_change,"
                "positive_streak,initial_stabilization_streak,rearm_streak,"
                "initial_arming_complete,rate_warmup_remaining,"
                "candidate_active,candidate_phase,"
                "candidate_validation_blocks,candidate_positive_streak,"
                "rapid_minus_control_score,candidate_threshold\n"
            )
        ...
        with open(self.regime_log_path, "w") as f:
            f.write(
                "step,suspicion_step,rapid_minus_control_score,validation_threshold,"
                "validation_streak,validation_blocks\n"
            )

        with open(self.candidate_log_path, "w") as f:
            f.write(
                "step,event,suspicion_step,phase,rapid_minus_control_score,"
                "threshold,validation_streak,validation_blocks,"
                "forward_null_n,forward_null_mean,forward_null_std\n"
            )
        ...
        # -------------------------------------------------
        # State-action-familiarity-weighted MAGNITUDE detector
        #
        # Raw fast-vs-slow log-likelihood advantage:
        #   D_t = log p_fast - log p_slow
        #
        # Do NOT binarize D_t at zero. Small positive advantages are a
        # normal consequence of the fast predictor tracking online learning
        # slightly ahead of the EMA reference. Instead use a dead zone and
        # bounded magnitude:
        #
        #   e_t = clip(
        #       (D_t - evidence_margin)
        #       / (evidence_cap - evidence_margin),
        #       0, 1
        #   )
        #
        # Thus D_t <= margin contributes no regime evidence, while a large
        # sustained fast-over-slow advantage approaches one.
        #
        # Aggregate correlated transitions into blocks using explicit
        # historical state-action familiarity w_t:
        #
        #   b_k = sum_t w_t e_t / sum_t w_t
        #
        # Track short and long EMAs of the BLOCK evidence:
        #   q_short <- beta_s q_short + (1-beta_s) b_k
        #   q_long  <- beta_l q_long  + (1-beta_l) b_k
        #
        # and detect persistent positive elevation
        #   D_k = q_short - q_long.
        #
        # IMPORTANT: this is only a SUSPICION trigger. A final regime
        # change is declared only by held-forward candidate validation.
        #
        # Because e_t remains in [0,1], the existing rate-gap thresholds
        # remain on the same numerical scale as before.
        # -------------------------------------------------
        ...
        self.evidence_margin = cfg.evidence_margin
        self.evidence_cap = cfg.evidence_cap
        ...
        # -------------------------------------------------
        # Matched-control held-forward confirmation test
        # -------------------------------------------------
        # The fast/slow magnitude detector is only a CHEAP SUSPICION
        # trigger. A suspicion is adjudicated by TWO temporary learners
        # that start from exactly the same predictor weights AND optimizer
        # state and receive exactly the same future adaptation samples and
        # exactly the same Poisson bootstrap weights:
        #
        #   normal control: learning rate = predictor_lr
        #   rapid candidate: learning rate = predictor_lr * multiplier
        #
        # During validation BOTH are frozen and scored on later transitions
        # that neither model has trained on. Therefore the confirmation
        # statistic is not "learning versus not learning". It asks whether
        # deliberately rapid adaptation provides persistent forward
        # predictive improvement beyond ordinary streaming learning.
        self.candidate_adapt_steps = cfg.candidate_adapt_steps
        self.candidate_validate_steps = cfg.candidate_validate_steps
        self.candidate_max_validation_blocks = (
            cfg.candidate_max_validation_blocks
        )
        self.candidate_required_positive_blocks = (
            cfg.candidate_required_positive_blocks
        )
        self.candidate_fixed_threshold = (
            cfg.candidate_fixed_threshold
        )
        self.candidate_null_k = cfg.candidate_null_k
        self.candidate_null_min_blocks = (
            cfg.candidate_null_min_blocks
        )
        self.candidate_gap_clip = cfg.candidate_gap_clip
        self.candidate_control_lr = float(cfg.predictor_lr)
        self.candidate_rapid_lr = (
            float(cfg.predictor_lr)
            * float(cfg.candidate_rapid_lr_multiplier)
        )

        # Dedicated RNG. One Poisson draw per ensemble member is shared by
        # control and rapid candidate on every adaptation transition. This
        # matched bootstrap removes random resampling as a confound.
        self.candidate_rng = np.random.RandomState(
            cfg.seed + 104729
        )

        # Online null calibration. Only validation scores from REJECTED
        # matched tests are added, so confirmed regime changes do not
        # contaminate the stationary baseline.
        self.forward_null_stats = RunningStats()

        # At most one matched test exists at a time. Two temporary ensembles
        # are O(1) memory with respect to lifetime and number of regimes.
        self.candidate_active = False
        self.candidate_control_predictors = None
        self.candidate_rapid_predictors = None
        self.candidate_control_opts = None
        self.candidate_rapid_opts = None
        self.candidate_phase = "idle"
        self.candidate_phase_step = 0
        self.candidate_start_step = -1
        self.candidate_validation_blocks = 0
        self.candidate_positive_streak = 0
        self.candidate_last_score = float("nan")
        self.candidate_last_threshold = (
            self.candidate_fixed_threshold
        )
        self.candidate_validation_scores = []
        self.candidate_val_weighted_gap = 0.0
        self.candidate_val_weight_sum = 0.0
        self.candidate_val_count = 0
        ...
        if self.evidence_margin < 0.0:
            raise ValueError("Require evidence_margin >= 0")

        if self.evidence_cap <= self.evidence_margin:
            raise ValueError(
                "Require evidence_cap > evidence_margin"
            )
        ...
        if self.candidate_adapt_steps <= 0:
            raise ValueError("Require candidate_adapt_steps > 0")

        if self.candidate_validate_steps <= 0:
            raise ValueError("Require candidate_validate_steps > 0")

        if self.candidate_max_validation_blocks <= 0:
            raise ValueError(
                "Require candidate_max_validation_blocks > 0"
            )
        ...
        if not (
            ...
            raise ValueError(
                "Require 1 <= candidate_required_positive_blocks "
                "<= candidate_max_validation_blocks"
            )

        if self.candidate_fixed_threshold < 0.0:
            raise ValueError(
                "Require candidate_fixed_threshold >= 0"
            )

        if self.candidate_null_k < 0.0:
            raise ValueError("Require candidate_null_k >= 0")

        if self.candidate_null_min_blocks < 0:
            raise ValueError(
                "Require candidate_null_min_blocks >= 0"
            )

        if self.candidate_gap_clip <= 0.0:
            raise ValueError("Require candidate_gap_clip > 0")

        if self.candidate_control_lr <= 0.0:
            raise ValueError("Require control learning rate > 0")

        if self.candidate_rapid_lr <= self.candidate_control_lr:
            raise ValueError(
                "Require rapid candidate learning rate > normal control "
                "learning rate"
            )
        ...
        self.block_weighted_evidence = 0.0
    ...
    # =====================================================
    # Matched-control held-forward validation helpers
    # =====================================================

    def _freeze_module_list(self, predictors):
        for predictor in predictors:
            predictor.eval()
            for param in predictor.parameters():
                param.requires_grad_(False)

    def _ensemble_log_prob(
        self,
        predictors,
        raw_obs,
        action,
        ...
        """Moment-matched ensemble log probability for one transition."""
        ...
        with torch.no_grad():
            for predictor in predictors:
                ...
                means.append(mean_n)
                variances.append(torch.exp(logvar_n))
            ...
            mu = means.mean(dim=0)
            var = (
                (variances + means.pow(2)).mean(dim=0)
                - mu.pow(2)
            )
            var = torch.clamp(var, min=1e-8)
    ...
    def _candidate_threshold(self):
        """
        Held-forward confirmation threshold.

        Before enough rejected tests exist, use a conservative fixed
        threshold. Once a stationary null is available, require the score
        to exceed both the fixed floor and mean + k * std of rejected-test
        validation blocks.
        """
        ...
        if (
            self.forward_null_stats.n
            >= self.candidate_null_min_blocks
        ):
            calibrated = (
                self.forward_null_stats.mean
                + self.candidate_null_k
                * self.forward_null_stats.std
            )
            threshold = max(
                threshold,
                float(calibrated),
    ...
    def _log_candidate_event(
        ...
        null_mean = (
            self.forward_null_stats.mean
            if self.forward_null_stats.n > 0
            else float("nan")
        )
        null_std = (
            self.forward_null_stats.std
            if self.forward_null_stats.n > 1
            else float("nan")
        )

        with open(self.candidate_log_path, "a") as f:
            f.write(
                f"{self.steps},"
                f"{event},"
                f"{self.candidate_start_step},"
                f"{self.candidate_phase},"
                f"{validation_score:.8f},"
                f"{threshold:.8f},"
                f"{self.candidate_positive_streak},"
                f"{self.candidate_validation_blocks},"
                f"{self.forward_null_stats.n},"
                f"{null_mean:.8f},"
                f"{null_std:.8f}\n"
            )

    def _clone_optimizer_for_predictor(
        self,
        source_optimizer,
        predictor,
        lr,
        ...
        """
        Clone Adam state onto a copied predictor, then set a requested LR.

        Both matched learners therefore begin with the same optimizer
        moments as the live fast predictor at the suspicion time.
        """
    ...
    def start_candidate_test(self):
        """
        Start a matched normal-vs-rapid held-forward test.

        The normal control and rapid candidate start IDENTICALLY: same
        predictor parameters and same Adam state copied from the current
        main fast ensemble. During adaptation they see the exact same
        transitions and Poisson bootstrap weights; only their learning
        rates differ. During validation both are frozen and evaluated on
        future samples before either can train on them.
        """

        if self.candidate_active:
            return

        self.candidate_control_predictors = copy.deepcopy(
            self.fast_predictors
        )
        self.candidate_rapid_predictors = copy.deepcopy(
            self.fast_predictors
        )

        for predictors in (
            self.candidate_control_predictors,
            self.candidate_rapid_predictors,
        ):
            for predictor in predictors:
                predictor.train()
                for param in predictor.parameters():
        ...
        self.candidate_control_opts = []
        self.candidate_rapid_opts = []

        for n in range(self.num_predictors):
            self.candidate_control_opts.append(
                self._clone_optimizer_for_predictor(
                    self.fast_pred_opts[n],
                    self.candidate_control_predictors[n],
                    self.candidate_control_lr,
                )
            )
            self.candidate_rapid_opts.append(
                self._clone_optimizer_for_predictor(
                    self.fast_pred_opts[n],
                    self.candidate_rapid_predictors[n],
                    self.candidate_rapid_lr,
                )
            )

        self.candidate_active = True
        self.candidate_phase = "adapt"
        self.candidate_phase_step = 0
        self.candidate_start_step = self.steps
        self.candidate_validation_blocks = 0
        self.candidate_positive_streak = 0
        self.candidate_last_score = float("nan")
        self.candidate_last_threshold = (
        ...
        # A suspicion is not yet a regime change. Disable new cheap triggers
        # while this controlled experiment is active.
        self.detector_armed = False
        self.positive_streak = 0
        self.rearm_streak = 0

        self._log_candidate_event("START")

        print(
            f"MATCHED TEST started at step={self.steps}: "
            f"adapt={self.candidate_adapt_steps}, "
            f"validate={self.candidate_validate_steps}, "
            f"control_lr={self.candidate_control_lr:.6g}, "
            f"rapid_lr={self.candidate_rapid_lr:.6g}, "
            f"need={self.candidate_required_positive_blocks} "
            f"consecutive positive validation blocks"
        )

    def _train_predictor_with_weight(
        self,
        predictor,
        optimizer,
        raw_obs,
        ...
        """One strict-streaming predictor update with a supplied weight."""
    ...
    def _train_candidate_pair(
        ...
        """
        Train normal control and rapid candidate on the SAME transition.

        Each ensemble member receives the SAME Poisson bootstrap draw in
        both arms; the intended treatment difference is only learning rate.
        """

        for n in range(self.num_predictors):
            k_n_t = int(self.candidate_rng.poisson(1.0))

            self._train_predictor_with_weight(
                self.candidate_control_predictors[n],
                self.candidate_control_opts[n],
                raw_obs,
                action,
                target,
                k_n_t,
    ...
    def _reset_candidate_validation_accumulator(self):
        self.candidate_val_weighted_gap = 0.0
        self.candidate_val_weight_sum = 0.0
        self.candidate_val_count = 0

    def _reset_cheap_detector_after_test(
        self,
        keep_armed,
    ):
        """Remove stale suspicion evidence after a candidate decision."""

        self.block_weighted_evidence = 0.0
        self.block_weight_sum = 0.0
        self.block_count = 0

        # Start the short/long comparison again from a neutral baseline.
        self.q_short = None
        self.q_long = None
        self.positive_streak = 0
        self.rearm_streak = 0
        ...
        self.detector_armed = bool(keep_armed)

    def _discard_candidate_test(self):
        self.candidate_active = False
        self.candidate_control_predictors = None
        self.candidate_rapid_predictors = None
        self.candidate_control_opts = None
        ...
        self.candidate_phase = "idle"
        self.candidate_phase_step = 0
        self._reset_candidate_validation_accumulator()

    def reject_candidate_test(self):
        """Reject a suspicion and use its forward scores as null data."""

        scores = list(self.candidate_validation_scores)

        self._log_candidate_event(
            "REJECT",
            self.candidate_last_score,
            self.candidate_last_threshold,
        )

        for score in scores:
            self.forward_null_stats.update(score)

        print(
            f"MATCHED TEST rejected at step={self.steps}: "
            f"last_score={self.candidate_last_score:.3f}, "
            f"threshold={self.candidate_last_threshold:.3f}, "
            f"null_n={self.forward_null_stats.n}"
        )

        self._discard_candidate_test()

        # The suspicion was false. Keep the cheap trigger available, but
        # remove the old short/long excursion so it cannot immediately fire
        # again on exactly the same accumulated evidence.
        self._reset_cheap_detector_after_test(
            keep_armed=True,
        )

    def confirm_candidate_test(self):
        """
        Confirm a regime change and promote the forward-validated candidate.
        """

        suspicion_step = self.candidate_start_step
        score = self.candidate_last_score
        threshold = self.candidate_last_threshold
        streak = self.candidate_positive_streak
        nblocks = self.candidate_validation_blocks
        ...
        # The rapid arm has demonstrated forward predictive improvement
        # beyond normal adaptation, so promote it to the main fast ensemble.
        # Preserve its Adam moments, but restore the normal streaming LR.
        new_fast_opts = []
        ...
        for n, (
            ...
            fast_predictor.load_state_dict(
                rapid_predictor.state_dict()
            )
            ...
            optimizer.load_state_dict(
                copy.deepcopy(
                    self.candidate_rapid_opts[n].state_dict()
                )
            )
            for param_group in optimizer.param_groups:
                param_group["lr"] = float(self.cfg.predictor_lr)

            new_fast_opts.append(optimizer)

        self.fast_pred_opts = new_fast_opts

        # The old slow reference has completed its job. Begin a new
        # comparison epoch from the newly validated predictor.
        ...
        print(
            f"REGIME CHANGE confirmed at step={self.steps}: "
            f"suspicion_step={suspicion_step}, "
            f"rapid_minus_control={score:.3f}, "
            f"threshold={threshold:.3f}, "
            f"streak={streak}/{nblocks}"
        )

        self._discard_candidate_test()

        # A confirmed change starts a fresh detector epoch. Familiarity
        # counts are deliberately preserved so recurring support is not lost.
        self._reset_cheap_detector_after_test(
            keep_armed=False,
        )
    ...
    def update_candidate_test(
        ...
        """
        Advance the held-forward test by one transition.

        Returns:
            "confirm"  -> a regime change was confirmed
            "reject"   -> the suspicion was rejected
            None       -> test still running / no test
        """

        if not self.candidate_active:
            return None

        # The transition that CREATED the suspicion belongs to the cheap
        # trigger block. Start the held-forward experiment strictly from the
        # next transition.
        if self.steps <= self.candidate_start_step:
            return None

        if self.candidate_phase == "adapt":
            self._train_candidate_pair(
                raw_obs,
                action,
                target,
            )
            self.candidate_phase_step += 1

            if (
                self.candidate_phase_step
                >= self.candidate_adapt_steps
            ):
                self.candidate_phase = "validate"
                self.candidate_phase_step = 0
                self._reset_candidate_validation_accumulator()
                self._log_candidate_event("VALIDATE_START")

            return None

        if self.candidate_phase != "validate":
            raise RuntimeError(
                f"Unknown candidate phase: {self.candidate_phase}"
            )

        # HELD-FORWARD matched scoring. BOTH arms are frozen on this
        # validation transition. Since both were trained on exactly the same
        # earlier samples with the same bootstrap weights, ordinary streaming
        # learning is controlled for. Positive evidence means rapid
        # adaptation generalized better than normal adaptation.
        log_p_rapid = self._ensemble_log_prob(
            self.candidate_rapid_predictors,
            raw_obs,
            action,
            target,
        )
        log_p_control = self._ensemble_log_prob(
            self.candidate_control_predictors,
            raw_obs,
            action,
            target,
        ...
        forward_gap = float(
            np.clip(
                log_p_rapid - log_p_control,
                -self.candidate_gap_clip,
                self.candidate_gap_clip,
            )
        )

        weight = float(familiarity_weight)
        self.candidate_val_weighted_gap += (
            weight * forward_gap
        )
        self.candidate_val_weight_sum += weight
        self.candidate_val_count += 1
        self.candidate_phase_step += 1

        if (
            self.candidate_phase_step
            < self.candidate_validate_steps
        ):
            return None

        block_familiarity = (
            self.candidate_val_weight_sum
            / max(1, self.candidate_val_count)
        )

        if self.candidate_val_weight_sum > 1e-8:
            score = (
                self.candidate_val_weighted_gap
                / self.candidate_val_weight_sum
            )
        ...
        valid = (
            self.candidate_val_weight_sum > 1e-8
            and block_familiarity
            >= self.min_block_familiarity
        )

        self.candidate_validation_blocks += 1
        self.candidate_last_score = float(score)
        self.candidate_last_threshold = float(threshold)

        if valid:
            self.candidate_validation_scores.append(
                float(score)
            )

        if valid and score > threshold:
            self.candidate_positive_streak += 1
        else:
            self.candidate_positive_streak = 0
        ...
        print(
            f"MATCHED validation at step={self.steps}: "
            f"score={score:.3f}, threshold={threshold:.3f}, "
            f"familiarity={block_familiarity:.3f}, "
            f"streak={self.candidate_positive_streak}, "
            f"block={self.candidate_validation_blocks}/"
            f"{self.candidate_max_validation_blocks}"
        )

        if (
            self.candidate_positive_streak
            >= self.candidate_required_positive_blocks
        ):
            self.confirm_candidate_test()
            return "confirm"

        if (
            self.candidate_validation_blocks
            >= self.candidate_max_validation_blocks
        ):
            self.reject_candidate_test()
            return "reject"

        # Another cycle: train on NEW future samples, then validate again on
        # still-later samples that have not been used for matched-test updates.
        self.candidate_phase = "adapt"
        self.candidate_phase_step = 0
        self._reset_candidate_validation_accumulator()
        self._log_candidate_event("ADAPT_RESTART")
    ...
    def update(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            # Retain the old binary indicator for diagnostics only.
            fast_wins = int(
                fast_slow_gap > 0.0
            )

            # Magnitude-aware regime evidence.
            #
            # Small positive gaps are intentionally ignored because the
            # fast predictor normally stays slightly ahead of its slow EMA
            # reference during ordinary online learning.
            gap_evidence = float(
                np.clip(
                    (
                        fast_slow_gap
        ...
        # =================================================
        # 3. Familiarity-weighted block magnitude detector
        #
        # Per transition:
        #   w_t in [0,1] = state-action familiarity
        #   e_t in [0,1] = magnitude-aware fast-over-slow evidence
        #
        # The block statistic is
        #
        #   b_k = sum_t w_t e_t / sum_t w_t.
        #
        # This suppresses two different confounds:
        #   1) poorly supported (S_t, A_t) inputs via w_t;
        #   2) tiny ordinary fast-over-slow advantages via the dead zone.
        #
        # The block may update the detector only when its average
        # familiarity is sufficiently large.
        # =================================================
        ...
        if (
            ...
            if self.block_count >= self.block_size:
                ...
                if block_valid:
                    ...
                    if self.q_short is None:
                        self.q_short = block_evidence
                        self.q_long = block_evidence
                    else:
                        self.q_short = (
                        ...
                        if self.rate_warmup_remaining > 0:
                            ...
                            if (
                                ...
                                print(
                                    ...
                                    f"step={self.steps}: "
                                    f"weighted_block_evidence={block_evidence:.3f}, "
                    ...
                    # -------------------------------------
                        ...
                        if (
                            ...
                            print(
                                f"CHANGE SUSPICION at step={self.steps}: "
                                f"weighted_block_evidence={block_evidence:.3f}, "
                    ...
                    # -------------------------------------
                        ...
                        if self.candidate_active:
                            # A suspicion is currently being adjudicated by
                            # held-forward validation. Do not allow the cheap
                            # detector to rearm underneath it.
                            self.rearm_streak = 0
                ...
                with open(
                    ...
                    f.write(
                        ...
                        f"{int(self.candidate_active)},"
                        f"{self.candidate_phase},"
                        f"{self.candidate_validation_blocks},"
                        f"{self.candidate_positive_streak},"
                        f"{self.candidate_last_score:.8f},"
                        f"{self.candidate_last_threshold:.8f}\n"
                    )

        # =================================================
        # 4. Matched-control held-forward test
        # =================================================

        candidate_result = self.update_candidate_test(
            raw_obs=raw_obs,
        ...
        if candidate_result == "confirm":
            regime_change = True

        # =================================================
        # 5. Update ONLY the main fast ensemble on the current
        ...
        for n, predictor in enumerate(
            ...
            # Recompute after held-forward adjudication because a confirmed
            # candidate may have been promoted earlier in this same update.
            mean_n, logvar_n = predictor(
                raw_obs,
                detector_action,
            )
        ...
        # =================================================
        # 6. Update the slow reference AFTER the fast update.
        #
        # Startup warmup:
        #     slow <- fast exactly
        #
        ...
        if (
            ...
            with open(
                ...
                f.write(
                    ...
                    f"{int(self.candidate_active)},"
                    f"{self.candidate_phase},"
                    f"{self.candidate_phase_step},"
                    f"{self.candidate_validation_blocks},"
                    f"{self.candidate_positive_streak},"
                    f"{self.candidate_last_score:.8f},"
                    f"{self.candidate_last_threshold:.8f},"
                    f"{self.forward_null_stats.n},"
                    f"{(self.forward_null_stats.mean if self.forward_null_stats.n > 0 else float('nan')):.8f},"
                    f"{(self.forward_null_stats.std if self.forward_null_stats.n > 1 else float('nan')):.8f},"
        ...
        return {
            ...
            "candidate_phase_step": self.candidate_phase_step,
            "candidate_validation_blocks": (
                self.candidate_validation_blocks
            ),
            "candidate_positive_streak": (
                self.candidate_positive_streak
            ),
            "candidate_last_score": self.candidate_last_score,
            "candidate_threshold": self.candidate_last_threshold,
    ...
    def save(self, model_dir, unique_str):
        ...
        model = {
            ...
            "warmup_remaining": self.warmup_remaining,
            "block_weighted_evidence": self.block_weighted_evidence,
            ...
            "candidate_phase": self.candidate_phase,
            "candidate_phase_step": self.candidate_phase_step,
            "candidate_start_step": self.candidate_start_step,
            "candidate_validation_blocks": (
                self.candidate_validation_blocks
            ),
            "candidate_positive_streak": (
            ...
            "candidate_last_score": self.candidate_last_score,
            "candidate_last_threshold": (
                self.candidate_last_threshold
            ),
            "candidate_validation_scores": list(
                self.candidate_validation_scores
            ),
            "forward_null_n": self.forward_null_stats.n,
            "forward_null_mean": self.forward_null_stats.mean,
            "forward_null_M2": self.forward_null_stats.M2,
            "candidate_control_predictors": (
                [
                    predictor.state_dict()
                    for predictor in self.candidate_control_predictors
                ]
                if self.candidate_control_predictors is not None
                else None
            ),
            "candidate_rapid_predictors": (
                [
                    predictor.state_dict()
                    for predictor in self.candidate_rapid_predictors
                ]
                if self.candidate_rapid_predictors is not None
                else None
            ),
            "rate_warmup_remaining": self.rate_warmup_remaining,
            "detector_armed": self.detector_armed,
...
def main(args):
    ...
    run_id = (
        ...
        +
        f"-joint-matched-control-forward-validation"
        f"-{args.algo}"
        f"-{args.env}"
        f"_seed-{args.seed}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Number of transitions aggregated into one magnitude-"
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "EMA coefficient for the short-timescale block magnitude evidence"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "EMA coefficient for the long-timescale block magnitude evidence"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Dead zone for fast-minus-slow log-likelihood advantage. "
            "D_t <= margin contributes zero regime evidence."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Fast-minus-slow log-likelihood advantage mapped to full "
            "evidence. Evidence is linearly scaled between margin and cap."
        ),
    )

    # =====================================================
    # Matched-control held-forward confirmation
    # =====================================================

    parser.add_argument(
        "--candidate_adapt_steps",
        default=1000,
        type=int,
        help=(
            "Transitions used to train BOTH normal-control and rapid "
            "candidate before each held-forward validation segment"
        ),
    )

    parser.add_argument(
        "--candidate_validate_steps",
        default=1000,
        type=int,
        help=(
            "Future transitions scored by BOTH frozen matched arms; neither "
            "arm is trained on these samples"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="Maximum held-forward validation blocks per suspicion",
    )

    parser.add_argument(
        "--candidate_required_positive_blocks",
        default=2,
        type=int,
        help=(
            "Consecutive positive held-forward validation blocks required "
            "to confirm a regime change"
        ),
    )

    parser.add_argument(
        "--candidate_fixed_threshold",
        default=0.5,
        type=float,
        help=(
            "Minimum familiarity-weighted mean rapid-minus-control "
            "log-likelihood advantage required on a validation block"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "After enough rejected tests, threshold also must exceed "
            "null_mean + k * null_std"
        ),
    )

    parser.add_argument(
        "--candidate_null_min_blocks",
        default=5,
        type=int,
        help=(
            "Number of rejected-test validation blocks required before "
            "online null calibration is used"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Symmetric clip for per-transition held-forward "
            "rapid-minus-control log-likelihood gaps"
        ),
    )

    parser.add_argument(
        "--candidate_rapid_lr_multiplier",
        "--candidate_lr_multiplier",
        dest="candidate_rapid_lr_multiplier",
        default=4.0,
        type=float,
        help=(
            "Rapid-candidate learning-rate multiplier relative to the "
            "normal control. Both arms otherwise receive matched samples, "
            "bootstrap weights, initial parameters, and Adam state."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "Detectable elevation in short-minus-long block magnitude evidence"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Number of consecutive cheap-detector blocks for which the "
            "rate gap must exceed threshold before opening a candidate test"
        ),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "After a detection, the current weighted block magnitude evidence must "
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_joint_matched_control_forward_validation"
