...
class StateActionFamiliarityTracker:
    ...
    The detector uses only the positive part as its familiarity weight:

        w_t = max(0, z^F_t)

    Thus state-action regions less familiar than the regime's current
    visitation distribution contribute no change evidence, while very
    familiar regions can receive weights greater than one.
    """
...
class AVG:
    ...
    def update(
        ...
        **kwargs,
        ...
        # Relative familiarity gate.
        #
        # The old F_t = 1 - exp(-c_t / tau) saturated near one once a
        # bucket had been visited moderately often, so very different
        # visitation frequencies became nearly indistinguishable.
        #
        # Here z^F_t remains unbounded.  Only state-action regions that
        # are more familiar than the current regime-local reference
        # contribute detector evidence.
        if familiarity_ready:
            familiarity_weight = max(
                0.0,
                familiarity_z,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "z-scoring no longer uses F^gamma."
        ),
    )
