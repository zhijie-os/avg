"""Oracle-mixture pretraining for the min-delay regimes (10M steps).

Functionally identical to run_min_delay_stationary.py, except the training
horizon: one AVG agent trains from scratch, entirely within a single
min-delay regime, for 10,000,000 steps with no regime switch.  These mature
regime-specific models (``model.pt``, saved by default) are meant to be
loaded later by an oracle regime-switching experiment that jumps directly to
the pretrained model of the current regime.
...
This is "B_x from scratch -> train for 10M steps", NOT "A -> B_x": a freshly
...
Outputs go to experiment ``oracle_pretrain_<regime>`` so they never overwrite
the 5M results of run_min_delay_stationary.py.

Usage:
    python3 run_min_delay_oracle_pretrain.py --regime A
    python3 run_min_delay_oracle_pretrain.py --regime B_joint --actuator-index 3
    python3 run_min_delay_oracle_pretrain.py --regime B_combined --smoke-test
"""
...
def main():
    ...
    args.experiment_name = f"oracle_pretrain_{args.regime}"
