...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.regime_log_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_regime_changes_aba_mass_local_weighted_affine.log",
        )

        # Periodic detector trace for debugging/plotting.
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_detector_trace_aba_mass_local_weighted_affine.csv",
        )
...
def main(args):
    ...
    # =====================================================
    # A -> B -> A torso-mass regime
    # =====================================================

    mass_body = "torso"
    mass_body_id = base_env.model.body(mass_body).id
    original_mass = float(base_env.model.body_mass[mass_body_id])
    changed_mass = original_mass * args.mass_scale
    ...
    try:
        ...
        for t in range(args.N):
            ...
            if t == 25_000:
                # A -> B: scale the torso body mass.
                base_env.model.body_mass[mass_body_id] = changed_mass

                print(
                    f"A -> B at t={t}: {mass_body} mass "
                    f"{original_mass} -> {changed_mass} "
                    f"(scale={args.mass_scale})"
                )

            elif t == 50_000:
                # B -> A: restore the original torso mass.
                base_env.model.body_mass[mass_body_id] = original_mass

                print(
                    f"B -> A at t={t}: {mass_body} mass "
                    f"{changed_mass} -> {original_mass}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=2.0,
        type=float,
        help="Torso-mass multiplier used in regime B; 2.0 doubles the torso mass",
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_mass_detector_local_weighted_affine"
