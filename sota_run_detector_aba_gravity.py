...
class AVG:
    ...
    def __init__(self, cfg):
        ...
        self.regime_log_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_regime_changes_aba_gravity_local_weighted_affine.log",
        )

        # Periodic detector trace for debugging/plotting.
        self.detector_trace_path = os.path.join(
            cfg.results_dir,
            f"{cfg.run_id}_detector_trace_aba_gravity_local_weighted_affine.csv",
        )
...
def main(args):
    ...
    # =====================================================
    # A -> B -> A gravity regime
    # =====================================================

    original_gravity = base_env.model.opt.gravity.copy()
    changed_gravity = original_gravity * args.gravity_scale
    ...
    try:
        ...
        for t in range(args.N):
            ...
            if t == 25_000:
                # A -> B: scale the full MuJoCo gravity vector.
                base_env.model.opt.gravity[:] = changed_gravity

                print(
                    f"A -> B at t={t}: gravity "
                    f"{original_gravity} -> {changed_gravity} "
                    f"(scale={args.gravity_scale})"
                )

            elif t == 50_000:
                # B -> A: restore the original gravity vector.
                base_env.model.opt.gravity[:] = original_gravity

                print(
                    f"B -> A at t={t}: gravity "
                    f"{changed_gravity} -> {original_gravity}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=2,
        type=float,
        help="Gravity multiplier used in regime B; 1.5 changes -9.81 m/s^2 to about -14.715 m/s^2",
    )
    ...
    pkl_fpath = os.path.join(
        ...
        (
            f"{args.env}"
            f"_aba_gravity_detector_local_weighted_affine"
